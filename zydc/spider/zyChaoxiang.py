...
#创建一个一维数组赋值给a
a = np.arange(1,len(zyPandas.chaoxiang_group)+1,1)
...
#设置坐标轴的刻度
plt.xlim(0,7000)
plt.ylim(0,len(zyPandas.chaoxiang_group)+1)
#设置图表标题
...
#设置y轴数据分类名称
plt.yticks(a,tuple(zyPandas.chaoxiang_group.index))
#保存图表
#plt.savefig('zyImage/chaoxiang.png')
plt.savefig('static/images/chaoxiang.png')
#显示图表
#plt.show()
plt.close()